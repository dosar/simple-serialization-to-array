...
object Length {
  ...
  def instanceF[T](f: T => Int) = new Length[T] {
    override def length(t: T) = f(t)
  }

  def instance[T](c: Int) = new Length[T] {
    override def length(t: T) = c
  }
